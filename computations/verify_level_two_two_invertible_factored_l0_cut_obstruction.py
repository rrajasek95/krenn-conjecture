#!/usr/bin/env python3
"""Exact cut-rank obstruction for the 2I+2R+2Z incidence survivor.
...
SOURCE = HERE / "verify_level_two_two_invertible_l0_incidence_survivor.py"
LEFT = (0, 1)
RIGHT = (2, 3, 4, 5)
VARIABLES = tuple(f"l{site}" for site in range(6))
EXPECTED_MINORS = 224
EXPECTED_NONZERO_GENERATORS = 80
...
def audit_unit_ideal(executable, characteristic, cut, label):
    ...
    require(payload == (str(EXPECTED_NONZERO_GENERATORS), "1", "1"),
...
def main():
    ...
    source["guard"]["audit_generic_kernel_and_selected_rows"]()
    source["guard"]["audit_rank_and_kernel"]()
    source["guard"]["audit_r2"]()
    ...
    print("two-invertible factored L0 cut obstruction: all checks passed")
    ...
    print(
        "  cut-minor system       : "
        f"{EXPECTED_MINORS} cubics, {EXPECTED_NONZERO_GENERATORS} nonzero, "
        "in 6 gauges"
    )
