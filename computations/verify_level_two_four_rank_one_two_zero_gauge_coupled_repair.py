#!/usr/bin/env python3
"""Shared full-L0 rank-42 guards through the 4R+2Z endpoint pattern.

Repair the integrated gauge-coupled packet by adding pure-zero cells on
03 and 12 and the compensating 01 cell.  The relation

    M01(00) M23(00) + M03(00) M12(00) = 0

preserves the pure-one endpoint slice.  Roots 0, 1, 2, and 3 then have
complete R2 witness pairs.  Activate any subset on the common isotropic
input line e_0.  Standard library only; live under -O and -I -S.
"""

from itertools import combinations, product
from pathlib import Path
from runpy import run_path


def require(condition, message):
    if not condition:
        raise RuntimeError(message)


HERE = Path(__file__).resolve().parent
PREVIOUS = run_path(str(
    HERE / "verify_level_two_two_rank_one_four_zero_gauge_coupled_family.py"
))
RIGID = PREVIOUS["RIGID"]
BASE = PREVIOUS["BASE"]
CORE = PREVIOUS["CORE"]
SITES = PREVIOUS["SITES"]
COLOURS = PREVIOUS["COLOURS"]
EDGES = PREVIOUS["EDGES"]
ZERO_MATRIX = PREVIOUS["ZERO_MATRIX"]
CAPABLE = (0, 1, 2, 3)
OUTPUT_FACTORS = {
    0: (2, 3),
    1: (3, 5),
    2: (5, 7),
    3: (7, 11),
}
WITNESSES = {
    0: {0: 3, 1: 2},
    1: {0: 2, 1: 3},
    2: {0: 3, 1: 0},
    3: {0: 2, 1: 1},
}
POTENTIALS = (0,) * len(SITES)


def repaired_member():
    packet, u_star, v_star, parameters = RIGID["integrated_member"]()
    p, _q, _beta, _gamma, _left, _right = parameters
    a, b = 2, 1
    c = -(a * b) // p
    require(c * p + a * b == 0,
            "the three-edge pure-one cancellation changed")
    packet[0, 3, 0, 0] = a
    packet[1, 2, 0, 0] = b
    packet[0, 1, 0, 0] = c
    return packet, u_star, v_star, (a, b, c, p)


def selected_family(active):
    selected = {site: ZERO_MATRIX for site in SITES}
    for site in active:
        h0, h1 = OUTPUT_FACTORS[site]
        selected[site] = ((h0, 0), (h1, 0))
    return selected


def audit_rank_and_l0(packet, u_star, v_star):
    tangents = {
        (s, t): BASE["factored_tangent"](u_star, v_star, s, t)
        for s, t in product(COLOURS, repeat=2)
    }
    outputs = {
        key: CORE["apply_differential"](packet, tangent)
        for key, tangent in tangents.items()
    }
    require(outputs == {
        (0, 0): [int(word == (0,) * 6) for word in CORE["WORDS"]],
        (0, 1): [0] * 64,
        (1, 0): [0] * 64,
        (1, 1): [int(word == (1,) * 6) for word in CORE["WORDS"]],
    }, "the four-rank-one repair lost its four L0 slices")
    derivative = CORE["differential_matrix"](packet)
    mixed = [
        row for row, word in zip(derivative, CORE["WORDS"])
        if word not in ((0,) * 6, (1,) * 6)
    ]
    ranks = (
        BASE["ranks_over_fields"](derivative),
        BASE["ranks_over_fields"](mixed),
    )
    require(ranks == (
        (42, 42, 42, 42),
        (40, 40, 40, 40),
    ), ("the four-rank-one repaired ranks changed", ranks))
    return ranks


def audit_selected_equations(packet, selected):
    tangent = {}
    checks = 0
    for u, v in EDGES:
        for a, b in product(COLOURS, repeat=2):
            numerator = sum(
                selected[u][a][i] * selected[v][b][1 - i]
                for i in COLOURS
            )
            require(
                numerator
                == (POTENTIALS[u] + POTENTIALS[v]) * packet[u, v, a, b],
                ("four-rank-one generic-kernel identity failed",
                 u, v, a, b),
            )
            tangent[u, v, a, b] = numerator
            checks += 1
    require(checks == 60, "four-rank-one scalar census changed")
    require(CORE["apply_differential"](packet, tangent) == [0] * 64,
            "a four-rank-one selected row survived")
    return checks


def audit_literal_slices(packet, u_star, v_star, selected):
    function = BASE["audit_literal_eight_site_slices"]
    globals_dict = function.__globals__
    old_x = globals_dict["X"]
    try:
        globals_dict["X"] = selected
        checked = function(packet, u_star, v_star)
    finally:
        globals_dict["X"] = old_x
    require(checked == 256, "the four-rank-one literal slice census changed")
    return checked


def audit_capable_root(packet, root):
    table = {}
    for output, neighbour in WITNESSES[root].items():
        require(BASE["pure_internal_column"](packet, root, neighbour, output),
                ("a four-rank-one pure witness vanished",
                 root, output, neighbour))
        complement = tuple(
            site for site in SITES if site not in (root, neighbour)
        )
        nonzero = sum(
            CORE["hafnian"](packet, complement, word) != 0
            for word in CORE["WORDS"]
        )
        require(nonzero,
                ("a four-rank-one witness cofactor vanished",
                 root, output, neighbour))
        table[output] = (neighbour, nonzero)
    return table


def main():
    packet, u_star, v_star, repair = repaired_member()
    ranks = audit_rank_and_l0(packet, u_star, v_star)
    witness_tables = {
        root: audit_capable_root(packet, root) for root in CAPABLE
    }
    counts = {size: 0 for size in range(5)}
    results = {}
    for size in range(5):
        for active in combinations(CAPABLE, size):
            selected = selected_family(active)
            endpoint_ranks = tuple(
                CORE["rational_rank"](selected[site]) for site in SITES
            )
            require(sum(endpoint_ranks) == size,
                    ("four-rank-one endpoint ranks changed",
                     active, endpoint_ranks))
            generic = audit_selected_equations(packet, selected)
            literal = audit_literal_slices(packet, u_star, v_star, selected)
            for root in active:
                require(witness_tables[root],
                        ("an active four-rank-one root lost R2", active, root))
            for root in set(SITES) - set(active):
                require(selected[root] == ZERO_MATRIX,
                        ("an inactive root lost preservation", active, root))
            counts[size] += 1
            results[active] = (endpoint_ranks, generic, literal)
    require(counts == {0: 1, 1: 4, 2: 6, 3: 4, 4: 1},
            ("the four-rank-one subset census changed", counts))
    print("four-rank-one gauge-coupled repair: all checks passed")
    print(f"  repair (M03,M12,M01; M23): {repair}")
    print(f"  differential ranks          : {ranks}")
    print(f"  capable-root witnesses      : {witness_tables}")
    print(f"  active-subset census        : {counts}")
    print(f"  selected rank cases         : {len(results)}/16")
    print("  conclusion                  : shared full L0 reaches 4R+2Z at rank 42")


if __name__ == "__main__":
    main()
