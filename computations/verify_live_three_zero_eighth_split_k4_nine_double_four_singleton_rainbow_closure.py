...
def check_pair_drop_cores_and_lifts() -> None:
    assert sum(PROFILE) == TOTAL == 2 * H_SPLIT + K + 2
    ...
    for selected_doubles in combinations(double_indices, 3):
        ...
        for lowered_pair in combinations(formal_roles, 2):
            ...
            assert sum(takes.values()) == H_SPLIT
            assert sum(complement) == 14
            assert complement.count(1) == 2
            assert frontier.leaves_singleton(PROFILE, takes)
            assert represented == 7 - omitted_singletons
            assert residual_degree == 4 - omitted_singletons
            assert lift_degree == 4 + omitted_singletons
            assert residual_degree + lift_degree == 8
            core_count += 1

    assert core_count == sp.binomial(9, 3) * sp.binomial(7, 2) == 1764
...
def check_kernel_bound_and_parity_boundary() -> None:
    ...
    assert sp.Poly(parity_minor, z).degree() <= 15
    ...
    assert sp.Poly(ordinary_divisor, z).degree() == 15
    assert sp.Poly(zero_divisor, z).degree() == 15
    ...
    assert sp.diff(local_product, w).subs(w, 0) == b0 * g0 * e0
    ...
    for absorbed_singletons, (forced, cap) in table.items():
        ...
        assert forced == 2 * (4 - absorbed_singletons)
...
def check_relation_plane_and_outside_rows() -> None:
    ...
    assert sp.Poly(g_A, z).degree() == 6
    assert sp.Poly(radical, z).degree() == 6
    assert sp.Poly(D_A, z).degree() == 5
    ...
    for degree in range(8):
        ...
        if degree < 7:
            assert trial_differential.degree() == degree + 6
    ...
    assert sp.expand(row - expected) == 0
...
def check_pair_identities_and_boolean_differences() -> None:
    ...
    assert sp.expand(first_cross - (U * (1 - q) - 2 * common)) == 0
    assert sp.expand(second_cross - (V * (1 + p) - 2 * common)) == 0

    anchor, value = sp.symbols("anchor value")
    phi = 2 / (anchor + value) + 3 / (anchor - value)
    psi = -2 / (anchor + value) ** 2 - 3 / (anchor - value) ** 2
    assert sp.factor(phi - (5 * anchor + value) / (anchor**2 - value**2)) == 0
    assert sp.diff(2 / (anchor + value), anchor) - (
        -2 / (anchor + value) ** 2
    ) == 0
    assert sp.diff(3 / (anchor - value), anchor) - (
        -3 / (anchor - value) ** 2
    ) == 0
    assert psi == (
    ...
    assert sp.factor(third_differences[0] - expected_first) == 0
    assert sp.factor(third_differences[1] - expected_second) == 0
...
def check_fibres_and_six_vertex_deletion() -> None:
    ...
    h_u = x * y + 5 * u * (x + y) + u**2
    h_v = x * y + 5 * v * (x + y) + v**2
    assert sp.factor(h_u - h_v) == (
        u - v
    ) * (u + v + 5 * x + 5 * y)
    fixed_sum = -(u + v) / 5
    assert sp.expand(h_u.subs(x + y, fixed_sum)) == x * y - u * v
...
def check_rainbow_obstruction() -> None:
    ...
    assert len(edges) == 15
    assert len(perfect) == 15
    assert all(len(matching) == 3 for matching in perfect)
    assert all(
    ...
    assert exact_transversals == stars
    assert all(len(star) == 5 for star in stars)
    assert all(
        left & right
...
def check_profile_location() -> None:
    ...
    assert census.EXPECTED_RESIDUALS == ()
