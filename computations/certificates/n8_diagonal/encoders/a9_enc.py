...
# --------------------------------------------------------------- tool paths
# P2-diag edit (2026-08-20), promotion checklist item A11: the audit lane
# hard-coded an absolute path here.  The byte-identical original is preserved
# at `encoders/original/a9_enc.py`; NOTHING ELSE in this file is changed, and
# the CNF this module emits is unaffected (verified: the 87 regenerated CNFs
# still match the shipped SHA-256 digests).
#
# Resolution order, first hit wins:
#   1. the environment variables CADICAL / DRATTRIM (or DRAT_TRIM);
#   2. a tools directory relative to THIS file, via TOOLS_CANDIDATES;
#   3. the bare binary name, found on PATH;
#   4. the bare name anyway, so the failure is a clear "not found" at call
#      time rather than an import-time crash.
_HERE = os.path.dirname(os.path.abspath(__file__))

TOOLS_CANDIDATES = (
    os.environ.get("KG_TOOLS", ""),
    # certified_package/encoders/ -> computations/unaudited-hygiene-h1-*/tools
    os.path.join(_HERE, os.pardir, os.pardir, os.pardir,
                 "unaudited-hygiene-h1-2026-08-15", "tools"),
    # a promoted layout that keeps the tools beside the checker
    os.path.join(_HERE, os.pardir, "tools"),
    os.path.join(_HERE, "tools"),
)


def _resolve(env_names, relative, binary):
    for name in env_names:
        val = os.environ.get(name)
        if val:
            return val
    for root in TOOLS_CANDIDATES:
        if not root:
            continue
        cand = os.path.normpath(os.path.join(root, relative))
        if os.path.exists(cand):
            return cand
    return shutil.which(binary) or binary


TOOLS = next((os.path.normpath(r) for r in TOOLS_CANDIDATES
              if r and os.path.isdir(r)), "")
CADICAL = _resolve(("CADICAL",), "cadical/build/cadical", "cadical")
DRATTRIM = _resolve(("DRATTRIM", "DRAT_TRIM"), "drat-trim/drat-trim",
                    "drat-trim")
