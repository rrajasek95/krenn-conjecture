#!/usr/bin/env python3
"""Discover compact root-RUP palettes extending the 3+4+3 basis."""
...
import verify_n8_d1_m10_433_full_shadow as C
...
def main():
    ...
    if os.path.exists(C.PALETTE_PATH):
        ...
        with open(C.H.PALETTE_PATH, "rb") as handle:
            palettes = C.H.decode_palettes(json.loads(handle.read()))
    for index, _state in branches:
        ...
        with open(C.PALETTE_PATH, "wb") as handle:
            handle.write(raw)
        print("433:%d residuals=%d palettes=%d"
    ...
    if C.EXPECTED_PALETTE_SHA256 != "TO_BE_FROZEN":
        if digest != C.EXPECTED_PALETTE_SHA256:
            raise RuntimeError("the deterministic 4+3+3 palettes changed")
