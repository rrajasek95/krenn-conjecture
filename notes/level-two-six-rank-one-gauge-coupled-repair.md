...
relation, so it also preserves the slices.  The original endpoint stars
still give

\[
       (T_{00},T_{01},T_{10},T_{11})
                  =(e_{0^6},0,0,e_{1^6})                    \tag{6}
\]

literally, while the residual differential ranks rise to

\[
                 \operatorname{rank}D=50,\qquad
                 \operatorname{rank}D_{\rm mixed}=48.       \tag{7}
\]
...
all-rank-one case at rank \(50/48\).
...
verifies the cancellation relations, all four exact differential slices,
rational and
three-prime ranks, the twelve pure-column witnesses and their cofactors,
and all \(64\) literal eight-site active-subset cases.  It is intended to
run normally, with -O, and with -I -S.
