# An inactive doubly-good curved OO packet survives two diagonal anchors

## 1. Outcome

If arm activity is omitted, adding one differently labelled diagonal anchor to the
...
It fails the third diagonal target `X_1`, and both displayed OO arms are
inactive.  Therefore even two
...
cut or the missing target without using activity.  It does not show that
all three diagonal anchors are indispensable on the actual active stratum.
...
alter local ranks and curvature but cannot change the tensor.  In
particular, the two proposed OO arms `02,04` have zero deleted cofactors and
are not active.  Hence
...
derived only from OO rank/alignment/curvature data **without activity**.  It
does not disprove the corresponding active-arm statement.
...
discarding even one diagonal label leaves the present inactive packet.  The
honest next target is the two-anchor saturated ideal with one chosen nonzero
cofactor coordinate for each arm.  Only if that active ideal survives would
all three diagonal rows be known indispensable.
