...
which preserve the three complete old cuts (2,3,4) for nonzero (t).
...
of witnesses or exceptional parameter chart is needed.
...
2, those are boundary-touching directions.  The selected positive-degree
matching cofactors do not see their weights.  This suggests the next
source-independent lemma:

> after the constant layer is removed, the selected Fitting cofactor depends
> only on the cut-2 interior provenance class, modulo the ideal generated by
> boundary-supported exact deformations.
