...
## Projective kernel normal forms

The missing-kernel-label residue has a finite exact linear-algebra
classification.  Put

\[
 Q=D/\langle d\rangle,\qquad
 {\cal T}_A=\bigotimes_{x\in A}V_x,
\qquad
 \Phi_A:\operatorname{Sym}^2Q\longrightarrow{\cal T}_A,\quad
 \Phi_A(e^{[2]})=\Omega_A^S(\bar e).                        \tag{7}
\]

Because \(d_i=0\), the \(i\)-th physical coordinate descends to a nonzero
linear form \(\ell_i\in Q^*\).  The only direction which does not activate
the missing diagonal is the point \(\mathbb P(\ker\ell_i)\).  Hence a
no-descent packet must satisfy

\[
 \mathbb P(\ker\Phi_A)\cap\nu_2(\mathbb P Q)
 \ \subseteq\
 \nu_2\bigl(\mathbb P(\ker\ell_i)\bigr),                    \tag{8}
\]

where \(\nu_2\) is the Veronese conic of squares.

Choose a basis \(x,y\) of \(Q\) with
\(\ell_i(x)=1,\ell_i(y)=0\), so the inactive point is \([0:1]\).
Intersection with a nonsingular conic gives the following exhaustive
normal forms.

1. \(\operatorname{rank}\Phi_A=0\) is impossible.
2. If \(\operatorname{rank}\Phi_A=1\), its projective kernel line must be
   tangent to the Veronese conic at the inactive square.  Equivalently,
   for one \(T\ne0\),
   \[
      \Phi_A(x^{[2]})=T,\qquad
      \Phi_A(xy)=\Phi_A(y^{[2]})=0.                         \tag{9}
   \]
3. If \(\operatorname{rank}\Phi_A=2\), its one-dimensional vector kernel
   is either generated by a nonsquare binary quadratic, or by the inactive
   square \(y^{[2]}\).
4. If \(\operatorname{rank}\Phi_A=3\), the map is injective and has no
   projective basepoint.

Indeed, a projective line meets the Veronese conic in length two over
\(\mathbb C\).  If both intersection points are confined to the sole
inactive point, the line is its tangent.  A projective kernel point in the
rank-two case is either off the conic or is itself a square, and (8)
decides the latter case.  This identifies the precise next target: exclude
the tangent rank-one packet, the nonsquare/inactive rank-two kernels, and
the injective cofactor map using a literal overlap row.  An unqualified
claim that a vector-valued binary quadratic has a root would be false.
