...
overlap theorem.  The same guard already contains a projective
three-dimensional space of canonically clean caps, all inactive: the
direct scalar vanishes on the four-dimensional cap plane and every
polarized cubic response product is zero.  The rank-two linear map recorded
in the one-site note is only the contracted source-row residual, not the
canonical clean error.  Thus the overlap must force **activity**, leave the
scalar cap plane, or give a source contradiction—not merely another clean
relation.
