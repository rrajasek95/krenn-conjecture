...
overlap theorem.  The same guard already contains a projective line of
clean caps, all inactive: its full clean-error map has rank two on the
four-dimensional cap plane, with kernel generated by a unary clean cap and
the target-free cap.  Thus the overlap must force **activity**, not merely
