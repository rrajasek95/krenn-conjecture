...
| $f=6$ | $C_6$, $C_3\sqcup C_3$ | `proofs/saturated-rank-graph-obstruction.md` | all 54 exceptional entries forced nonzero in $C_6$, then rectangle minors; zero-chord elimination for $C_3\sqcup C_3$ before the rank argument | `computations/verify_saturated_rank_graph_obstruction.py` (the $C_3\sqcup C_3$ relaxation is UNSAT for every asymmetric anchor-colour orbit) |
