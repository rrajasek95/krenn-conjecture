...
4. The exact target-compatible cap locus at (pq) contains the projective
   ...
   [
          {cal L}=\operatorname {span}\{I,E_{12},E_{21}\}.
   ]
...
transverse pair rows fail.  Therefore it is not a counterexample to the
...
\[
 {cal C}_A(K)=K\mathbin{\lrcorner}H_8(A),\qquad
 {cal T}(K)=\sum_cK_{cc}X_c.
\]

Exact enumeration of the (729\times9) residual matrix
({\cal C}_A-{cal T}) gives rank six and
...
\[
             s=3\lambda,\qquad r=3\lambda PQ,
             \qquad(\kappa_0,\kappa_1,\kappa_2)
...
3. a coupling theorem showing that the curvature-selected direct line and
   an augmented E2 defect line have a common non-activity divisor.

Another four-site or Bianchi identity alone cannot supply that coupling.

The lightweight exact checker
[`verify_curved_good_pair_dirty_cap_line_guard.py`](../computations/verify_curved_good_pair_dirty_cap_line_guard.py)
