...
9. The canonical first `C6` is classified by optional response pairs:
   dense support is a unit, `E13` is a typed response chord, and the first
   survivor is response-silent `E14`, awaiting its physical hole-`14`
...
8. No single-face comparison exists in the complete first collision degree:
...
fine grade, and prove

```text
derived Yw -> physical W.
```

On the source side it must physically lift the five zero-anchor adjacent
collision boundaries.  It must not identify chart `-S_v` with anchor
incidence; the primitive anchor or annihilator is the subsequent Fredholm
output.
...
Its source input must include zero-anchor repeated-site collision cells and
their degree-five compatibility.  The rootless pentagon first syzygies have
`P3 disjoint-union K2` degree, while the existing normal/chart Hasse faces
are squarefree; a degree-preserving renaming cannot supply the comparison.
...
3. The naked regularized normal difference retains
   ...
   `-S_v`.
4. The nondense face-zero supports have an exact finite classification.
