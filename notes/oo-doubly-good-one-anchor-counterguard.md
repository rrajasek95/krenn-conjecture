# An inactive doubly-good curved OO packet survives one diagonal anchor

## 1. Outcome

Without the selected-arm activity hypotheses, the normalized `22/21` OO atom from
...
`X_2`.  Both direct-arm cofactors vanish, and it also fails the two missing
diagonal targets `X_0,X_1`, so it is not a ternary source and does not lie
in the active selected-witness OO stratum.  Thus neither one diagonal anchor plus one
off-diagonal coefficient, nor even one diagonal anchor plus **all six**
off-diagonal rows in both charts, closes the rank/alignment/curvature
relaxation when activity is omitted.  Activity may itself supply the
missing coupling.
...
After deleting `p,q`, the remaining sites `a,b` have lost their only
incident blocks; after deleting `p,r`, site `a` has lost its only incident
block.  Hence

\[
                  H_{B\setminus\{p,q\}}=0,
             \qquad H_{B\setminus\{p,r\}}=0.             \tag{11}
\]

The finite saturation proposed in the previous note is nonempty because it
omitted (11).  The
...
Before the activity omission was noticed, the smallest proposed OO input
was the threshold identified by the independent one-anchor selector guard:
...
The present packet dies as soon as either `X_0` or `X_1` is imposed.  The subsequent
[`two-anchor OO counterguard`](oo-doubly-good-two-anchor-counterguard.md)
also has inactive arms.  Neither packet decides the active two-anchor
stratum.  The corrected finite target must saturate by a nonzero coordinate
of each cofactor tensor, in addition to the four star minors and curvature.
