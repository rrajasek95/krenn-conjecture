...
The unresolved same-column chart is now confined to (6). The transverse
one-column chart in which the two missing selected columns are different
has (b_2^{\mathsf T}Jb_3\ne0) and therefore a determined nonzero (23)
block; it is not covered here. The zero-potential cross
(\nu_2+\nu_3=0), where (M_{23}) becomes an arbitrary free block, is also
separate. These are precise support boundaries for the next analysis.
