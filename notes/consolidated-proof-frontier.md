...
|---|---|---|
| [Common coloop](common-coloop-clean-cap-affine-fibre.md), \(b=1\) | The [anchor--polar response quotient](common-coloop-anchor-polar-response-quotient.md) and [polar-dual refinement](common-coloop-polar-dual-forced-diagonal-boundary.md) classify the exact failures. The [literal two-arm kernel criterion](common-coloop-two-arm-polar-kernel-boundary.md) closes both nonmissing interpolation strata when two fixed-scalar polar-kernel arms have independent \(A\)-coordinates. The [scalar-extended cokernel theorem](common-coloop-scalar-extended-polar-cokernel-boundary.md) retains scalar attainability before contraction and shows that, in the singleton \(7/9\) boundary, the two omitted arm rows are exactly the only untested scalar coefficients | Use both omitted singleton rows \((r,t)\), \((t,s)\) to defeat every augmented annihilator pair \((\Lambda,\nu)\) at some attainable \(z\ne0\). The sharp guards show that a tensor residual may otherwise use the wrong row provenance, while a diagonal-complete packet may kill the tensor residual but leave every nonzero scalar unattainable |
