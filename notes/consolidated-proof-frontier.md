...
|---|---|---|
| [Common coloop](common-coloop-clean-cap-affine-fibre.md), \(b=1\) | The [anchor--polar response-quotient theorem](common-coloop-anchor-polar-response-quotient.md) now makes the comparison source-faithful: at each attainable scalar, all three diagonal increments factor through the same tangent-response space; the full system is exactly one \(D_{\bar K}(z)\)-image incidence plus three induced \(A=q_0^{[h-1]}\) coordinate tests | Force some attainable \(z\ne0\) whose polar residual lies in that response image and for which none of the three induced anchor coordinates is forced to zero; the missing-row \(A\)-annihilations alone do not force the image incidence |
