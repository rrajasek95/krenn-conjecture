...
|---|---|---|
| [Common coloop](common-coloop-clean-cap-affine-fibre.md), \(b=1\) | The response-quotient and polar-dual reductions classify the exact failures, and the [two-arm kernel criterion](common-coloop-two-arm-polar-kernel-boundary.md) closes both nonmissing interpolation strata on its fixed-scalar subcase. The [scalar-extended theorem](common-coloop-scalar-extended-polar-cokernel-boundary.md) restores row provenance. Adding both missing rows gives the [cross-row coupling](common-coloop-cross-row-scalar-coupling-elimination.md): if \(Q=q^{[h]}\ne0\), then \(a_{rt}a_{ts}=0\); local rank at least two kills both coefficients, while local rank one leaves at most one source-provenant scalar arm | Close four exact augmented-cokernel residues: zero top with two invisible direct coefficients; local-rank-at-least-two with both \(A\)-arms zero; and the two one-sided rank-one branches \(\widetilde\alpha A=-Q\), \(\Lambda(\widetilde\alpha D)=\mu\). The literal \(7/9\) packet itself is excluded because neither cross tensor lies on the \(Q\)-line |
